...
#DATASET
df = pd.read_csv('datas/ecoclean2.csv', sep =',', error_bad_lines=False) 
df.set_index('Date', inplace = True)
df.index = pd.to_datetime(df.index)
df['Année'] = df['Année'].astype(str)

df_r = pd.read_csv('datas/regionclean2.csv', sep =',', error_bad_lines=False) 
df_r.drop('Unnamed: 0', axis=1, inplace=True)
df_r['Année']=df_r['Année'].astype(str)

datas=pd.read_csv('datas/datasdjuclean.csv', sep=',')
